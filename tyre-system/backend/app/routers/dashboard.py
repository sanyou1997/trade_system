...
async def get_daily_summary(
    ...
    summary = await dashboard_service.get_daily_summary(db, target_date)
...
async def get_wechat_message(
    ...
    msg = await dashboard_service.generate_wechat_message(db, target_date)
...
async def get_monthly_stats(
    ...
    stats = await dashboard_service.get_monthly_stats(db, year, month)
...
async def get_sales_trend(
    ...
    if not 1 <= month <= 12:
        return ApiResponse.fail("Month must be between 1 and 12")
    trend = await dashboard_service.get_sales_trend(db, year, month)
