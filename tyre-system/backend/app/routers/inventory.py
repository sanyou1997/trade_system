...
async def get_inventory(
    ...
    items = await inventory_service.get_inventory(db, year, month)
...
async def update_stock(
    ...
    inv = await inventory_service.update_stock(
        db,
...
async def get_low_stock(
    ...
    await ensure_inventory_exists(db, year, month)
    items = await inventory_service.get_low_stock(db, year, month, threshold)
    return ApiResponse.ok(items)
...
async def rollover_month(
    ...
    count = await inventory_service.rollover_month(
        db,
